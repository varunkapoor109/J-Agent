...
class JobSearchService {
  ...
  async searchJobs(resumeData) {
    ...
    if (this.jsearchApiKey) {
      try {
        const jobs = await this.searchJSearch(searchQueries);
    ...
    if (this.adzunaAppId && this.adzunaAppKey) {
      try {
        const jobs = await this.searchAdzuna(searchQueries);
        allJobs = [...allJobs, ...jobs];
      } catch (error) {
  ...
  buildSearchQueries(resumeData) {
    ...
    // Primary role search
    if (resumeData.primaryRole?.title) {
      queries.push(resumeData.primaryRole.title);
    }

    // Add role variations based on seniority
    ...
    const topSkills = resumeData.skills.technical.slice(0, 3);
    topSkills.forEach(skill => {
      queries.push(`${skill} ${resumeData.primaryRole?.type || 'Engineer'}`);
    });

    return [...new Set(queries)].slice(0, 5); // Limit to 5 unique queries
  }
